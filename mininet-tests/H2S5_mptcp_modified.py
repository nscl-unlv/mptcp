...
def under_testing():
    ...
    if config.ADD_LINK:
        ...
        print('link added\n')
    '''
    time.sleep(config.TEST_DURATION/5.0)
    if config.CUT_LINK:
        print('cutting link...')
        h1.intf('h1-eth1').ifconfig('down')
        h2.intf('h2-eth1').ifconfig('down')
        print('link down\n')

    time.sleep(config.TEST_DURATION/5.0)
    if config.ADD_LINK:
        print('adding a new link...\n')
        net.addLink(h1, s6, cls=TCLink,
                    # bw=config.BANDWIDTH,
                    max_queue_size=config.MAX_QUEUE_SIZE)
        net.addLink(h2, s6, cls=TCLink,
                    bw=config.BANDWIDTH,
                    max_queue_size=config.MAX_QUEUE_SIZE,
                    delay=config.DELAY)

        s3.attach('s6-eth0')  # add interface name
        h1.setIP('10.0.1.4', intf='h1-eth1') # replace cut link
        h2.setIP('10.0.2.4', intf='h2-eth1') # replace cut link

        h1.intf('h1-eth1').ifconfig('up')
        h2.intf('h2-eth1').ifconfig('up')

        h1.cmd('ip rule add from 10.0.1.4 table 2')
        h2.cmd('ip rule add from 10.0.2.4 table 2')

        h1.cmd('ip route add default via 10.0.1.4 dev h1-eth0 table 2')
        h2.cmd('ip route add default via 10.0.2.4 dev h2-eth0 table 2')
        print('link added\n')
    '''
