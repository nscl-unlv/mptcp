...
def start_test():
    ...
    h2.cmd('iperf3 -s -i 1.0 -f m > iperf_bandwith_server_log_3.txt &')

    print('starting iperf client at', h1.IP(), ', connect to ', h2.IP())
    h1.cmd(f'iperf3 -t {str(config.TEST_DURATION)} '
           f'-f m -i 1.0 -c {h2.IP()} '
           f'> iperf_bandwith_client_log_3.txt &')

    under_testing()

    print('iperf client response:')
    print(h1.cmd('cat iperf_bandwith_client_log_3.txt'))

    print('iperf server response:')
    print(h2.cmd('cat iperf_bandwith_server_log_3.txt'))
