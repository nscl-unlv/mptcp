...
for (( run=1; run<=$RUNS_PER_TEST; run++ )); do
    for (( size=0; size<len_sizes; size++ )); do
	    echo -e "\n\nSTARTING RUN $run: ${INTF2_RTTS[0]} delay on $INTF_1, ${FILE_SIZES[$size]}\n\n"
...
for (( rtt=1; rtt<$len_rtts; rtt++ )); do
    for (( size=0; size<len_sizes; size++ )); do
        ...
        sudo tc qdisc change dev $INTF_1 root netem delay ${INTF2_RTTS[$rtt]} loss $PACKET_LOSS%
        tc qdisc show dev $INTF_1

        for (( run=1; run<=$RUNS_PER_TEST; run++ )); do
            echo -e "\n\nSTARTING RUN $run: ${INTF2_RTTS[$rtt]} delay on $INTF_1, ${FILE_SIZES[$size]}\n\n"
