...
import time
import re
import plotly as ply	 # https://plot.ly/python/reference/#scatter
...
def parse_csv(file):
    ...
    for res in allres:
        if res[0]-prev > max_interval:
            # float range
            for i in [prev+max_interval+f*max_interval
                      for f in range(int((res[0] - prev) / max_interval))]:
...
def parse_iperf(file):
    max_interval = 1.01  # don't forget to adjust!
    rawres = ''
    with open(file, 'r') as f:
        rawres = f.read()
    allres = re.findall(r'\[\s*\d\]\s*([\d.]*)-\s*([\d.]*)\s*sec\s*[\d.]*\s*\w*\s*([\d.]*)\s*(\w*?)/sec',
                        rawres)

    normres = []
    for res in allres:
        # don't include the summary at the end
        if float(res[1]) - float(res[0]) < max_interval:
            multiplier = {
                'bits': 1,
                'Kbits': 1024,
                'Mbits': 1024*1024,
...
def run_test():
    ...
    # run bandwith test
    os.system('python3 mptcp_test.py')
...
def save_logs(file_name_prefix):
    ...
    with open('results/'+file_name_prefix + '_bandwith_server_log.csv',
              'w') as f:
        for row in csv:
            f.write(str(row[0])+' '+str(row[1])+'\n')

    csv = parse_iperf('iperf_bandwith_client_log.txt')
    with open('results/' + file_name_prefix + '_bandwith_client_log.csv',
              'w') as f:
        for row in csv:
            f.write(str(row[0])+' '+str(row[1])+'\n')

    os.system('cp iperf_bandwith_server_log.txt results/' + file_name_prefix +
              '_bandwith_server_log.txt')
    os.system('rm iperf_bandwith_server_log.txt')

    os.system('cp iperf_bandwith_client_log.txt results/' + file_name_prefix +
              '_bandwith_client_log.txt')
    os.system('rm iperf_bandwith_client_log.txt')


def create_plot(file_name_prefix, cc, pm, sc):

    server_bandwidth = parse_csv('results/' + file_name_prefix +
                                 '_bandwith_server_log.csv')
    ...
    layout = ply.graph_objs.Layout(title=title,
                                   yaxis=dict(title='bandwith', showgrid=False))
    ...
    fig.write_image('results/' + file_name_prefix + '.png')


for cc in [('', 'cubic'), ('mptcp_coupled', 'lia'), ('mptcp_olia', 'olia'),
           ('mptcp_wvegas', 'wvegas'), ('mptcp_balia', ' balia')]:
    if debug and cc[1] != 'olia':
        continue
    if cc[0]:
        os.system('modprobe '+cc[0])
    os.system('sysctl -w net.ipv4.tcp_congestion_control='+cc[1])

    for pm in [('', 'default'), ('', 'fullmesh'),
               ('mptcp_ndiffports', 'ndiffports'), ('mptcp_binder', 'binder')]:
        if debug and pm[1] != 'fullmesh':
            continue
        if pm[0]:
            os.system('modprobe '+pm[0])
        os.system('sysctl -w net.mptcp.mptcp_path_manager=' + pm[1])

        for sc in [('', 'default'), ('mptcp_rr', 'roundrobin'),
                   ('mptcp_redundant', 'redundant')]:
            if debug and sc[1] != 'default':
                continue
            if sc[0]:
            ...
            # in case the plotter failed at a point,
            # and have to re-run the script, avoid recreate existing plots,
            if os.path.exists('results/'+file_name_prefix+'.png'): 
                print('plot image already exists, skipping')
            else:
    ...
    for cc in [('', 'cubic'), ('mptcp_coupled', 'lia'), ('mptcp_olia', 'olia'),
        ...
        for pm in [('', 'default'), ('', 'fullmesh'),
            ...
            for sc in [('', 'default'), ('mptcp_rr', 'roundrobin'),
                ...
                f.write('<img src="results/' + str(file_name_prefix) +
                        '.png" />\n')
